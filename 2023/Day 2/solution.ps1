...
foreach ($game in $gameData){
    ...
    if ($finalRed -le $maxRed){
        $redCheck = $true
    }

    if ($finalGreen -le $maxGreen){
        $greenCheck = $true
    }

    if ($finalBlue -le $maxBlue){
        $blueCheck = $true
    }
    if ($redCheck -and $blueCheck -and $greenCheck){
        write-host "$($game.game) $redCheck $blueCheck $greenCheck $finalRed/$finalBlue/$finalGreen"
        $answer = $answer + $currentGame
    }
}
write-host $answer